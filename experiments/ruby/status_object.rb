  ...
  module ClassDefinitions
    ...
    def self.extended(base)
      base.instance_eval do
        ...
        values = const_get(:VALUES)

        {valss: values } # => {:valss=>{:imported=>0, :walked=>1, :worked=>2, :completed=>3, :hidden=>4}}

        values.each do |name, val|
          const_set(name.to_s.upcase.to_sym, val) # => 0, 1, 2, 3, 4
          base.define_method(name) { val }
        end
        ...
        def name_map
          @name_map ||= const_get(:VALUES).invert.to_mash
        end
