...
module FindPatternsInFilenames
  ...
  def run(args=ARGV.clone, inp=$stdin, out=$stdout)
    ...
    parser = DirectoryParser.new(args.shift || Dir.pwd)
    parser.parse
    parser.save_script
    parser.edit_script
  end
  ...
  class DirectoryParser
    attr_accessor :dir, :file_list, :files, :subdirs, :patterns, :multi_patterns, :pwd
    def initialize(dir=Dir.pwd)
      @dir = dir
      @file_list = Dir[File.join(dir, '*')]
    ...
    def destination(group=nil)
      if group
        File.join(dir, 'file_groups', group)
      else
        File.join(dir, 'file_groups')
      end
    end
    ...
    def scriptfile
      @scriptfile ||= File.join(dir, 'move_files.sh')
    end
    ...
    def edit_script
      cd_to dir
      system('vim', File.basename(scriptfile))
      cd_to pwd
    end
